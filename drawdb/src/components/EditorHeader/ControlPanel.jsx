...
export default function ControlPanel({
  ...
  // Fetch recently opened diagrams from backend
  useEffect(() => {
    const fetchRecentDiagrams = async () => {
      if (!isAuthenticated) {
        setRecentlyOpenedDiagrams([]);
        return;
      }
      ...
      try {
        ...
        const allDiagrams = [...ownDiagrams, ...sharedDiagrams]
          ...
          .slice(0, 10);
        setRecentlyOpenedDiagrams(allDiagrams);
      } catch (error) {
  ...
  const loadDiagram = async (id) => {
    if (!isAuthenticated) {
      Toast.error(t("login_required") || "Please log in to load diagrams");
      return;
    }
  ...
  const menu = {
    file: {
      ...
      delete_diagram: {
        ...
        function: async () => {
          if (!isAuthenticated) {
            Toast.error(t("login_required") || "Please log in to delete diagrams");
            return;
          }
          ...
          try {
            ...
            setRedoStack([]);
            setGistId("");
            Toast.success(t("diagram_deleted") || "Diagram deleted successfully");
          } catch (error) {
            console.error(error);
            Toast.error(t("oops_smth_went_wrong"));
          }
